...
WHERE G1.CodFiscale = C1.CodFiscale
  AND G1.CodFiscale NOT IN (SELECT CodFiscale
                            FROM CAUSA
                            WHERE TipologiaCausa = 'Diffamazione'
                              AND  DATE(DataInizio) <= '2020/12/31'
                              AND (Date(DataFine) >= '2020/01/01' OR DataFine IS NULL))
